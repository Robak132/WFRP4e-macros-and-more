export class RollTracker {
  getUserData(userId) {
    return {
      ...
      data: game.users.get(userId)?.getFlag("wfrp4e-macros-and-more", "data"),
      reroll: game.users.get(userId)?.getFlag("wfrp4e-macros-and-more", "reroll"),
      export: game.users.get(userId)?.getFlag("wfrp4e-macros-and-more", "export")
    };
  }
  ...
  async saveReRoll(userId, chatMessage) {
    if (game.userId === userId) {
      ...
      if (chatMessage.content.includes(game.i18n.localize("DARKDEAL.UsageText").replace("{character}", ""))) {
        ...
        chatMessage.content.includes(game.i18n.localize("FORTUNE.UsageRerollText").replace("{character}", ""))
      ) {
        rerollData.fortune++;
      } else if (
        chatMessage.content.includes(game.i18n.localize("FORTUNE.UsageAddSLText").replace("{character}", ""))
      ) {
        rerollData.fortuneSL++;
      }

      return Promise.all([game.users.get(userId)?.setFlag("wfrp4e-macros-and-more", "reroll", rerollData)]);
    }
  }

  async saveTrackedRoll(userId, chatMessage) {
    if (game.userId === userId) {
      let maxTrackedRolls = game.settings.get("wfrp4e-macros-and-more", "roll_storage");
      ...
      return Promise.all([game.users.get(userId)?.setFlag("wfrp4e-macros-and-more", "data", data)]);
    }
  }
  ...
  async clearTrackedRolls(userId) {
    return Promise.all([
      game.users.get(userId)?.unsetFlag("wfrp4e-macros-and-more", "export"),
      game.users.get(userId)?.unsetFlag("wfrp4e-macros-and-more", "data"),
      game.users.get(userId)?.unsetFlag("wfrp4e-macros-and-more", "reroll")
    ]);
  }
  ...
  prepareExportData(data) {
    ...
    game.users.get(game.userId)?.setFlag("wfrp4e-macros-and-more", "export", fileContent);
  }
  ...
  async generalComparison() {
    ...
    for (let user of game.users) {
      if (game.users.get(user.id)?.getFlag("wfrp4e-macros-and-more", "data")) {
  ...
  async prepMode(finalComparison, comparator, comparators, allStats) {
    ...
    for (let user of comparators.top) {
      ...
      finalComparison[comparator].highest.percentage = Math.round(
        (finalComparison[comparator].highest.value / finalComparison[comparator].highest.rolls) * 100
      );
    }
    ...
    for (let user of comparators.topPercentage) {
      ...
      finalComparison[comparator].highestPercentage.percentage = Math.round(
        (finalComparison[comparator].highestPercentage.value / finalComparison[comparator].highestPercentage.rolls) *
          100
      );
    }
  }
...
export class RollTrackerDialog extends FormApplication {
  constructor(userId, options = {}) {
    super(userId, options);
    console.log(game.users.get(userId)?.getFlag("wfrp4e-macros-and-more", "data"));
  }
  ...
  static get defaultOptions() {
    ...
    const overrides = {
      ...
      width: "400",
      id: "roll-tracker",
      template: `modules/wfrp4e-macros-and-more/templates/roll-tracker.hbs`
    };
    return foundry.utils.mergeObject(defaults, overrides);
  }

  get exportData() {
    return game.robakMacros.rollTracker.getUserData(game.userId)?.export;
  }

  async getData() {
    const rollData = game.robakMacros.rollTracker.prepareRollStats(this.object);
    this.options.title = `${rollData.username}: Roll Tracker`;
    return rollData;
  }

  async prepCompCard() {
    let comparison = await game.robakMacros.rollTracker.generalComparison();
    let content = await renderTemplate(
      `modules/wfrp4e-macros-and-more/templates/roll-tracker-comparison-card.hbs`,
      comparison
    );
    ...
    switch (action) {
      case "clear": {
        const confirmed = await Dialog.confirm({
          title: game.i18n.localize("MACROS-AND-MORE.confirms.clear_rolls.title"),
          content: game.i18n.localize("MACROS-AND-MORE.confirms.clear_rolls.content")
        });
        if (confirmed) {
          await game.robakMacros.rollTracker.clearTrackedRolls(userId);
          this.render();
        }
        break;
      }
      case "print": {
        ...
        const content = await renderTemplate(
          `modules/wfrp4e-macros-and-more/templates/roll-tracker-chat.hbs`,
          rollData
        );
        ChatMessage.create({content});
      }
