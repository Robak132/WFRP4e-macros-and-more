...
async function generate_book() {
  ...
  // Condition
  let bookCondition = await game.wfrp4e.tables.rollTable("books-condition", { "modifier": bookAge.object.flags.age.condition, "minOne": true, hideDSN: true });
  ...
  return {
    name: name, type: "trapping", img: "modules/wfrp4e-core/icons/equipment/book_documents/book-apothecary.png", data: {
      trappingType: {
...
for (let i=0;i<books_amount;i++) {
  ItemWfrp4e.create(await generate_book()).then(itl => {
    ChatMessage.create({
      content: "<h1>Random Books Generator</h1><p>@Item[" + itl.id + "]{" + itl.name + "}</p><p>" + itl.description.value + "</p>"
    }, false);
  });
}