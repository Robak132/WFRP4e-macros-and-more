...
const BOOK_TOPIC = [
  ...
  {min: 81, max: 98, result: {value: "Scholarship"}},
  {
    min: 99,
    max: 100,
    result: {value: "Forbidden, Exotic or Heretical Topic"}
  }
];

async function rollFromTableCode(table) {
  const roll = (await new Roll("1d100").roll()).total;
...
async function generate_book() {
  ...
  // Quality
  const bookQuality = await rollFromTableCode(BOOK_QUALITY);
  console.log(bookQuality);
  worthModifier += bookQuality.result.worth;

  // Topic
  const bookTopic = await rollFromTableCode(BOOK_TOPIC);
  console.log(bookTopic);

  // Pages
  const bookPages = (await new Roll("1d10").roll()).total * 50;
  console.log(bookPages);

  // Type
  const bookType = await game.wfrp4e.tables.rollTable("books-type", {
    hideDSN: true
  });
  console.log(bookType);

  // Age
  const bookAge = await game.wfrp4e.tables.rollTable("books-age", {
    hideDSN: true
  });
  const bookAgeDetailed = (await new Roll("1d10").roll()).total * bookAge.object.flags.age.modifier;
  console.log(bookAge);

  // Condition
  const bookCondition = await game.wfrp4e.tables.rollTable("books-condition", {
  ...
  // Features
  const bookFeatures = await game.wfrp4e.tables.rollTable("books-features", {
    hideDSN: true
  });
  console.log(bookFeatures);

  // Features
  const bookLanguage = await game.wfrp4e.tables.rollTable("books-language", {
    hideDSN: true
  });
  let bookLanguageResult = bookLanguage.result;
  if (bookLanguage.roll % 11 === 0 || bookLanguage.roll === 100) {
    const originalBookLanguage = await game.wfrp4e.tables.rollTable("books-original-language", {hideDSN: true});
    bookLanguageResult = `${bookLanguageResult} (Translated from ${originalBookLanguage.result})`;
  }
  console.log(bookLanguage);

  // Name
  const topic = bookTopic.result.value.toLowerCase();
  let name = "Random Book";
  switch (topic) {
    case "biography":
      let reverse = false;
      let element_1 = await game.wfrp4e.tables.rollTable(`books-${topic}-1`, {
        hideDSN: true
      });
      while (element_1.roll > 98) {
        element_1 = await game.wfrp4e.tables.rollTable(`books-${topic}-1`, {
          hideDSN: true
        });
        reverse = !reverse;
      }
      let element_2 = await game.wfrp4e.tables.rollTable(`books-${topic}-2`, {
        hideDSN: true
      });
      while (element_2.roll > 98) {
        element_2 = await game.wfrp4e.tables.rollTable(`books-${topic}-2`, {
          hideDSN: true
        });
        reverse = !reverse;
      }
      const element_3 = await game.wfrp4e.tables.rollTable(`books-${topic}-3`, {
        hideDSN: true
      });
      const element_4 = await game.wfrp4e.tables.rollTable(`books-${topic}-4`, {
        hideDSN: true
      });
