...
export default class MaintenanceWrapper extends FormApplication {
  async render() {
    ...
    await new Dialog(
      {
        title: `Maintenance`,
        content: await renderTemplate("modules/wfrp4e-macros-and-more/templates/maintenance.hbs", content),
        buttons: {
          macros: {
            label: "Update Macros",
            callback: async () => {
              // Delete macros
              await Macro.deleteDocuments(
                game.macros.filter((m) => m.folder?.name === `Robak's Macros`).map((m) => m.id)
              );
              // Delete Macro folder
              ...
              Utility.log(
                await game.packs.get(`wfrp4e-macros-and-more.macros`).importAll({
  ...
  async #buildLocalizedContent(documentType) {
    const toolkitContent = documentType
      .filter((m) => m.folder?.name === `Robak's Macros`)
