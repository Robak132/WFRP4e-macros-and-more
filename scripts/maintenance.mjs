...
export default class MaintenanceWrapper extends FormApplication {
  async render() {
    ...
    await new Dialog({
      ...
      content: await renderTemplate("modules/wfrp4e-macros-and-more/templates/maintenance.html", content), buttons: {
        update: {
          label: "Update Macros", callback: async () => await this.createOrUpdateDocuments(),
        },
      },
    }, {popOut: true, width: 560, height: 200 + content.macros.length * 22}).render(true);
  }
  ...
  async #buildLocalizedContent() {
    ...
    if (existingFolder) {
      for (let content of existingMacros) {
        ...
        content.compendiumVersion = compendiumMacros.find(d => d.id === content.id)?.flags["wfrp4e-macros-and-more"]?.version ?? "null";
  ...
  async createOrUpdateDocuments() {
    ...
    if (!existingFolder) {
      existingFolder = await Folder.create({name: `Robak's Macros`, type: "Macro"});
    }
    ...
    for (let doc of existingDocuments) {
      if (game.macros.get(doc.id).flags["wfrp4e-macros-and-more"]?.version !== doc.flags["wfrp4e-macros-and-more"]?.version) {
    ...
    ui.notifications.notify(`${game.i18n.format("UPDATER.Notification", {
      created: newDocuments.length, updated: toUpdate.length, name: module.id, version: module.version,
    })}`);
  }
