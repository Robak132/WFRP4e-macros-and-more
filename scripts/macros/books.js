...
const BOOK_TYPE_PRINTED = {type: "Printed", enc: 1, baseValue: 1};
const BOOK_TYPE_ILLUMINATED = {type: "Illuminated", enc: 1, baseValue: 3};
const BOOK_TYPE_ILLUSTRATED = {type: "Illustrated", enc: 1, baseValue: 2};
...
const SCHOLAR_ELEM_III = [
  ...
  {min: 27, max: 28, data: async () => ({text: `${await generateLanguage()} Lexicon`})},
...
const TITLE_GENERATORS = {
  Biography: () => generateBiographyData(),
  Bestiary: () => generateBestiaryData(),
  Cookbook: () => generateCookbookData(),
  Guidebook: () => generateGuidebookData(),
  Fiction: () => generateFictionData(),
  Religious: () => generateReligiousData(),
  Scholarship: () => generateScholarshipData(),
...
async function generateBook(options = {}) {
  ...
  return {
    ...
    bookType: type.type,
    encumbrance: type.enc * (1 + encMult),
    age,
    condition,
    notableFeatures,
    penmanshipPeculiarities,
    origin,
    author,
...
async function generateLibrary(typeKey, size, options = {}) {
  ...
  for (const [topic, info] of entries) {
    ...
    for (let i = 0; i < info.size; i++) {
      info.books.push(await generateBook({classification: topic, ...options}));
    }
  }
...
function presentDescription(book) {
  ...
  let conditionFeatures = [
    ...
    book.condition?.illegible ? `${book.condition.illegible} of the text is illegible` : null,
    book.condition?.spineBroken ? `spine is broken` : null,
    book.condition?.missingPages ? `${book.condition.missingPages} pages are missing` : null,
    book.condition?.missingCover ? `cover is missing` : null
  ]
    .filter(Boolean)
    .join(", ");
...
function presentGMDescription(book) {
  ...
  if (book.synopsis) items.push(`<li><strong>Content:</strong> ${book.synopsis}</li>`);
  if (book.species || book.speciesFocus) {
    items.push(`<li><strong>Species focus:</strong> ${book.speciesFocus || book.species}</li>`);
  }
  if (book.penmanshipPeculiarities?.length) {
    items.push(
      `<li><strong>Penmanship:</strong> ${book.penmanshipPeculiarities
        .map((p) => p.description)
        .filter(Boolean)
        .join("; ")}</li>`
    );
  ...
  if (book.insanity) {
    items.push(`<li><strong>Danger:</strong> Prolonged reading may cause mental instability or corruption</li>`);
  }
